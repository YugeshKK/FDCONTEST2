    ...
    const element = arr[index];
     ...
     for (const key in element) {
      if (Object.hasOwnProperty.call(element, key)) {
        if(key=='first_name'){
          var first = element[key];
        }
        if(key=='last_name'){
          var last= element[key];
        }
        const hh= first+" "+ last;
        fullname.push(hh);

        var ans= fullname.filter((peyar)=>{
         ...
         if(ans.length>0){
            for (const key in arr) {
              if (arr.hasOwnProperty.call(arr, key)) {
                ...
                const modhal= element.first_name;
                const last= element.last_name;
                var mulu= modhal+" "+ last;
                if(mulu==ans[0]){
                  // console.log(element);
                  const vishyam=[];
                 for (const key in element) {
                  if (element.hasOwnProperty.call(element, key)) {
                    if(key=='id'){
                      var roll= element[key];
                    }
                    if(key=='first_name'){
                      var first=element[key];
                    }
                    if(key=='last_name'){
                      var lasst= element[key];
                    }
                    if(key=='gender'){
                      var gen= element[key]
                    }
                    if(key=='class'){
                      var clas=element[key];
                    }
                    if(key=='marks'){
                      var mar= element[key];
                    }
                    ...
                    vishyam.push(roll); vishyam.push(full); vishyam.push(gen);vishyam.push(clas); vishyam.push(mar); vishyam.push(pass); vishyam.push(em);

                    const trr= document.createElement('tr');

                    for (let index = 0; index <7; index++) {
                      var th= document.createElement('th');
                      th.innerText=vishyam[index];
                      trr.appendChild(th);       
                }
